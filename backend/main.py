...
from .routing.rtz import (
    route_to_rtz
)

from .routing.fuel import (
    VesselProfile,
    estimate_fuel
)
...
from .utils.daylight import (
    calculate_daylight_status
)
...
from .services.nmea_service import (
    nmea_service
)
...
async def nmea_websocket(
    websocket: WebSocket
):

    await websocket.accept()

    await nmea_service.register(
        websocket
    )
        ...
        while True:
            ...
            if position is None:
                ...
                await websocket.send_json(
                    {
                        "status": "error",
                        "message": "Invalid or unsupported NMEA sentence."
                    }
                )
            ...
            cursor.execute(
                ...
                (
                    validated.latitude,
                    validated.longitude,
                    validated.speed_knots,
                    validated.heading_degrees,
                    datetime.now(
                        timezone.utc
                    ).isoformat()
                )
            ...
            await nmea_service.broadcast(
                validated.model_dump()
            )

    except WebSocketDisconnect:

        await nmea_service.unregister(
            websocket
        )
...
def add_iceberg(
    ...
    cursor.execute(
        ...
        (
            iceberg.iceberg_id,
            iceberg.current_latitude,
            iceberg.current_longitude,
            iceberg.timestamp,
            iceberg.length_m,
            iceberg.width_m,
            iceberg.freeboard_m,
            iceberg.shape_class,
            iceberg.estimated_draft_m,
            iceberg.draft_uncertainty_m,
            iceberg.drift_speed_knots,
            iceberg.drift_direction_degrees,
            iceberg.predicted_latitude,
            iceberg.predicted_longitude,
            iceberg.forecast_time,
            iceberg.bias_corrected_latitude,
            iceberg.bias_corrected_longitude,
            int(
                iceberg.bias_correction_applied
            ),
            iceberg.status,
            iceberg.confidence,
            iceberg.source,
...
def get_environmental_values(
    ...
    return (
        sum(ice_values) / len(ice_values)
        if ice_values
        else 0.0,

        sum(wave_values) / len(wave_values)
        if wave_values
        else 0.0
    )
...
def calculate_route(
    ...
    grid = create_ocean_grid(
        ...
        sea_ice_data=sea_ice_rows,
        hazards=hazard_rows
    )

    start = find_nearest_grid_point(
        grid,
        request.start_latitude,
        request.start_longitude
    )

    goal = find_nearest_grid_point(
        grid,
        request.destination_latitude,
        request.destination_longitude
    )
    ...
    if path is None:
        ...
        raise HTTPException(
            status_code=404,
            detail="No safe route could be found."
        )
    ...
    for row, col in path:
        ...
        route_points.append(
            {
                "latitude": grid[row][col][
                    "latitude"
                ],
                "longitude": grid[row][col][
                    "longitude"
                ]
            }
        )
    ...
    risk_level = calculate_route_risk(
        path,
        grid
    )
    ...
    route = {
        ...
        "average_ice_concentration": round(
            average_ice,
            4
        ),

        "average_wave_height_m": round(
            average_wave,
            3
        ),
...
def calculate_vessel_fuel_profile(
    vessel: VesselProfileRequest
):

    profile = VesselProfile(
        vessel_id=vessel.vessel_id,
        draft_m=vessel.draft_m,
        fuel_rate_l_per_km=vessel.fuel_rate_l_per_km,
    ...
    return {
        ...
        "vessel": {
            ...
            "fuel_rate_l_per_km": profile.fuel_rate_l_per_km,
            "ice_class": profile.ice_class
        }
    }
...
def daylight(
    ...
    result = calculate_daylight_status(
        latitude,
        longitude,
        date.today()
    )
