...
def validate_raster(raster: np.ndarray) -> None:
    """
    Validate that the input raster is a 2D numerical array.
    """
...
def pixel_to_coordinates(
    ...
    """
    Convert raster row/column coordinates to projected coordinates.
...
def raster_to_polygons(
    ...
    source_crs: str = "EPSG:3031",
    target_crs: str = "EPSG:4326",
    min_area: float = 0.0,
) -> List[Dict[str, Any]]:
    """
    Convert a raster into polygons representing areas where the
    raster value is greater than or equal to the threshold.

    Parameters
    ----------
    raster:
        2D raster containing sea-ice concentration values.

    transform:
        (x_origin, y_origin, pixel_width, pixel_height)

    threshold:
        Minimum raster value to convert into a polygon.

    source_crs:
        CRS of the input raster.

    target_crs:
        CRS for the resulting GeoJSON coordinates.

    min_area:
        Minimum polygon area in target/projected coordinate space.
    """
...
def raster_to_geojson(
    ...
    """
    Convert a raster directly into a GeoJSON FeatureCollection.
    """
